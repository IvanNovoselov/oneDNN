...
template <cpu_isa_t isa, data_type_t acc_type, data_type_t dst_type>
void jit_pp_kernel_t<isa, acc_type, dst_type>::copy_floats(const Xbyak::Reg64& dst,
    ...
    foreach(rsi, 1, size, [&, this](const Xbyak::Reg64& idx) {
        mov(r13d, dword[src + idx * sizeof(float)]);
        mov(dword[dst + idx * sizeof(float)], r13d);
    });
                ...
                if (utils::one_of(isa, avx512_core_bf16, avx512_common)) {
                    ...
                    if (apply_mask) {
                        if (isa != sse41) {
                            ...
                            copy_floats(reg_dst, r8, reg_tmp);
                            add(rsp, vlen * sizeof(float));
